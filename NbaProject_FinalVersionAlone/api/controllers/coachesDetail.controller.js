...
const getOne = function (req, res) {
  ...
  Nba.findById(nbaId)
    ...
    .catch(function (err) {
      console.log("Error to find coache using" + " " + coachesId);
...
const deleteOne = (req, res) => {
  ...
  if (response.status != 200) {
    ...
    Nba.findById(nbaId).exec((err, nba) =>
      _deleteCoaches(err, nba, req, res, coa)
    );
    // console.log("nbaId" + " " + nbaId);
    // Nba.findById(nbaId)
    //   .exec()
    //   .then(function (nba) {
    //     _deleteCoaches(err, nba, req, res, coa);
    //   })
    //   .catch(function (err) {
    //     console.log("Error delete a coache");
    //     res.status(500).json(err);
    //   });
  }
};
